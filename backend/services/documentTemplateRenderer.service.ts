...
import { DocumentTemplateService, pageDimensions } from "./documentTemplate.service";
...
function pickFont(family: string | undefined, bold?: string, italic?: string): StandardFonts {
  const fam = String(family || "times")
    .toLowerCase()
    .replace(/[^a-z]/g, "");
  const table =
    FONT_MAP[fam] ||
    (fam === "timesnewroman" || fam === "timesroman" || fam === "serif" ? FONT_MAP.times : FONT_MAP.times);
...
async function loadImage(doc: PDFDocument, source: string): Promise<PDFImage | null> {
  ...
  try {
    if (/^data:image\/(png|jpeg|jpg);base64,/i.test(source)) {
      ...
      return meta[1].toLowerCase().includes("png") ? await doc.embedPng(bytes) : await doc.embedJpg(bytes);
    }
    const res = await fetch(source);
    if (!res.ok) return null;
    const bytes = Buffer.from(await res.arrayBuffer());
    const isPng = source.toLowerCase().includes(".png") || bytes.subarray(0, 4).toString("hex") === "89504e47";
...
export class DocumentTemplateRenderer {
  static async renderPDF(
    ...
    const template = await DocumentTemplateService.getByIdOrType(templateIdOrType);
    if (!template) throw new Error("Template not found");

    const { width, height } = pageDimensions(template);
    const pdfDoc = await PDFDocument.create();
    const page = pdfDoc.addPage([width, height]);
    const ctx = await DocumentTemplateService.buildFieldContext(opts.data || {});
    ...
    if ((template.page as any).background) {
      ...
      if (bg) {
        ...
        page.drawImage(bg, { x: 0, y: 0, width: iw * scale, height: ih * scale, opacity: 0.95 });
      }
    }
  ...
  private static async renderElement(
    ...
    switch (el.type) {
      case "text": {
        const resolved = DocumentTemplateService.resolveTemplateText(el.content || "", ctx);
        await this.drawText(pdfDoc, page, el, resolved, size, color);
        break;
      }
      case "dynamicText": {
        const value = el.field ? DocumentTemplateService.getDot(ctx, el.field) : "";
        await this.drawText(pdfDoc, page, el, value === undefined ? "" : String(value), size, color);
        break;
      }
      case "image": {
      ...
      case "signature": {
        ...
        const official = (await Official.findOne({ position, status: "active" }).lean().catch(() => null)) as any;
      ...
      case "rect": {
        this.drawBox(page, el.x, el.y, el.width, el.height, el.strokeColor || "#000000", el.strokeWidth || 1, el.backgroundColor);
        break;
      }
      case "table": {
        ...
        for (const row of rows) {
          ...
          const value = DocumentTemplateService.resolveTemplateText(String(row.value || ""), ctx);
          page.drawText(label, { x: el.x, y: cy - 12, size: 10, font: boldFont, color: rgb(0, 0, 0) });
          page.drawText(value, { x: el.x + el.width / 2, y: cy - 12, size: 10, font, color: rgb(0, 0, 0), maxWidth: el.width / 2 });
          page.drawLine({ start: { x: el.x, y: cy - 4 }, end: { x: el.x + el.width, y: cy - 4 }, thickness: 0.5, color: rgb(0.7, 0.7, 0.7) });
          cy -= 18;
        }
        break;
      }
  ...
  private static drawBox(page: PDFPage, x: number, y: number, w: number, h: number, color: string, borderWidth = 1, fill?: string) {
