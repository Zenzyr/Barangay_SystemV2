...
export function NotificationBell({
  ...
  const markAll = useMutation({
    mutationFn: () =>
      axiosInstance.patch(`/notification/${userId}/read-all`),
    onSuccess: () =>
  ...
  const markOne = useMutation({
    mutationFn: (id: string) =>
      axiosInstance.patch(`/notification/${id}/read`),
    onSuccess: () =>
      queryClient.invalidateQueries({
        queryKey: ["notifications", userId],
      }),
  ...
  return (
    <div
      ...
      className={cn(
        "relative",
        position === "desktop" && "hidden lg:block"
      )}
    >
      <button
        type="button"
      ...
      {open && (
        <div
          className={cn(
            "fixed left-1/2 top-16 z-[60] w-[calc(100vw-2rem)] max-w-sm -translate-x-1/2",

            "lg:absolute lg:left-auto lg:right-0 lg:top-full lg:mt-2 lg:w-[calc(100vw-2rem)] lg:max-w-sm lg:translate-x-0",

            "overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-2xl"
          )}
        >
          {/* Header */}
          ...
          <div className="max-h-[28rem] overflow-y-auto divide-y divide-slate-50">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center gap-2 px-4 py-10 text-center text-slate-400">
                ...
                <p className="text-xs">
                  Updates on your requests and account will appear here.
                </p>
              </div>
            ) : (
              items.map((item) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      !item.read && markOne.mutate(item._id)
                    }
                    className={cn(
